...
class SelectTest extends RsORMTest\Base {
    ...
    public function test() {
        ...
        $this->assertSame("SELECT DISTINCT HIGH_PRIORITY SQL_NO_CACHE id, name FROM table WHERE (id = ?) OR (id = ?) GROUP BY id HAVING alive = ? ORDER BY id DESC LIMIT ? OFFSET ?", $stmt->prepare());
