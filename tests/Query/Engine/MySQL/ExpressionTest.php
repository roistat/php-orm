...
class BetweenTest extends RSDBTest\Base {
    ...
    public function testLogicalOrExpressions() {
        $expr1 = new Expression\In(new Expression\Column("id"), [new Expression\Value(1), new Expression\Value(2), new Expression\Value(3)]);
        $expr2 = new Expression\Equal(new Expression\Column("alive"), new Expression\Value(1));
        $expr = new Expression\LogicalOr([$expr2, $expr1]);
        $this->assertSame("(`alive` = ?) OR (`id` IN (?, ?, ?))", $expr->prepare());
        $this->assertSame([1, 1, 2, 3], $expr->values());
    }
    ...
    public function testLogicalAndExpressions() {
        ...
        $expr2 = new Expression\Equal(new Expression\Column("alive"), new Expression\Value(1));
        $expr = new Expression\LogicalOr([$expr2, $expr1]);
        $this->assertSame("(`alive` = ?) AND (`id` IN (?, ?, ?))", $expr->prepare());
        $this->assertSame([1, 1, 2, 3], $expr->values());
    }
